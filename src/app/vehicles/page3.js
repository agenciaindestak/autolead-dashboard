...
export default function VehiclesPage() {
  ...
  async function uploadPhotos(vehicleId) {
    ...
    for (const photo of photos) {
      try {
        ...
        await fetch(`${API}/api/upload/photo/${vehicleId}`, {
          method: 'POST',
          body: formData
        })
      } catch (err) {
  ...
  async function saveVehicle() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const vehicleId = editId || saved.id
        if (photos.length > 0) {
  ...
  const s = {
    ...
    modal: { background: '#1f2937', borderRadius: '16px', padding: '2rem', width: '600px', maxHeight: '90vh', overflowY: 'auto' },
    ...
    photoGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem', marginTop: '0.75rem' },
    photoThumb: { position: 'relative', aspectRatio: '1', borderRadius: '8px', overflow: 'hidden', background: '#111827' },
    photoImg: { width: '100%', height: '100%', objectFit: 'cover' },
    photoRemove: { position: 'absolute', top: '4px', right: '4px', background: 'rgba(220,38,38,0.9)', color: '#fff', border: 'none', borderRadius: '50%', width: '20px', height: '20px', cursor: 'pointer', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1 },
    uploadArea: { border: '2px dashed #374151', borderRadius: '8px', padding: '1.5rem', textAlign: 'center', cursor: 'pointer', marginTop: '0.5rem' },
    sectionLabel: { fontSize: '0.75rem', color: '#9ca3af', marginBottom: '6px', display: 'block', marginTop: '1rem' }
  }
          ...
          <div key={v.id} style={s.card}>
            <div style={s.cardLeft}>
              ...
              <div>
                ...
                <div style={s.cardSub}>{Number(v.km || 0).toLocaleString('pt-BR')} km • {v.color} • {v.fuel}</div>
                {v.description && <div style={s.cardDesc}>{v.description.slice(0, 80)}...</div>}
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
              <div>
                <span style={s.price}>R$ {Number(v.price || 0).toLocaleString('pt-BR')}</span>
      ...
      {showForm && (
        <div style={s.overlay}>
          <div style={s.modal}>
            ...
            <div>
              <span style={s.sectionLabel}>Fotos do veículo</span>
              <input
                type="file"
                accept="image/*"
                multiple
              ...
              <div style={s.uploadArea} onClick={() => fileInputRef.current?.click()}>
                ...
                <div style={{ color: '#6b7280', fontSize: '0.75rem', marginTop: '4px' }}>
                  JPG, PNG — múltiplas fotos permitidas
                </div>
              </div>
            ...
            <div style={s.modalFooter}>
              <button style={s.btnGray} onClick={() => { setShowForm(false); setEditId(null); setPhotos([]); setPhotoPreviews([]) }}>
                Cancelar
              </button>
              <button style={s.btn} onClick={saveVehicle} disabled={saving || uploadingPhotos}>
                {uploadingPhotos ? '📤 Enviando fotos...' : saving ? 'Salvando...' : editId ? 'Atualizar' : 'Salvar Veículo'}
              </button>
            </div>
          </div>
        </div>
